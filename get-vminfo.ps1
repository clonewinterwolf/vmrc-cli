...
foreach ($vm in $vmlist)
{
    ...
    $vm|Select-Object @{N="VM";E={$vm.Name}},
                ...
                @{N='IP Address';E={[string]::Join(',',($_.Guest.IPAddress | Where-object {($_.Split(".")).length -eq 4}))}},
                @{N='DNS';E={[string]::Join(',',$_.ExtensionData.Guest.net.dnsconfig.IpAddress)}},
                @{N='Gateway';E={[string]::Join(',',($vm.ExtensionData.Guest.IpStack.IpRouteConfig.IpRoute | foreach-object{if($_.Gateway.IpAddress){$_.Gateway.IpAddress}}))}}|out-host
    ...
    $array_vmdk|select-object HardDisk,VMXpath,ProvisionType,CapacityGB,ThinUsage,Controller,DiskType|format-table -AutoSize|out-host
